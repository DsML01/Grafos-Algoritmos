...
//argv - Os parametros em si

void dfs(int vertice, vector<bool> &visitado, vector<vector<int>> grafo, stack<int> &pilha)
{
    visitado[vertice] = 1;

    for(auto i: grafo[vertice]) if(!visitado[i]) dfs(i, visitado, grafo, pilha);
...
void dfs_reverso(int vertice, vector<bool> &visitado, vector<vector<int>> grafo_reverso, vector<int> &componentes_fortemente_conexas)
{
    ...
    for(auto i: grafo_reverso[vertice]) if(!visitado[i]) dfs_reverso(i, visitado, grafo_reverso, componentes_fortemente_conexas);
}
...
void kosaraju(vector<vector<int>> grafo, vector<vector<int>> grafo_reverso, vector<vector<int>> &componentes_fortemente_conexas)
{
    ...
    for(int i = 1; i < visitado.size(); i++) if(!visitado[i]) dfs(i, visitado, grafo, pilha);

    for(int i = 1; i < grafo_reverso.size(); i++) visitado[i] = 0;

    while(!pilha.empty())
    {
        //retorna o item do topo sem remove-lo da pilha
...
int main(int argc, char **argv)
{
    ...
    //Se der algumerro, mudar o argv[++i] para argv[i + 1]
    for (int i = 1; i < argc; i++)
    {
        ...
        else if (strcmp(argv[i], "-f") == 0) input_file = argv[++i];
    }
    ...
    if (output_file != "null")
    {
        ...
        for (const auto &cfc : componentes_fortemente_conexas)
        {
            ...
            for (int node : cfc)
            {
                if (contador == cfc.size() - 1) fout << node;
